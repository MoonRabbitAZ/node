// Copyright 2018-2020 moonrabbit Technologies (UK) Ltd.
...
//! As part of moonrabbit's availability system, certain pieces of data
...
use moonrabbit_primitives::v0::{self, Hash as H256, BlakeTwo256, HashT};
use moonrabbit_node_primitives::AvailableData;
...
impl<'a, I: Iterator<Item=&'a [u8]>> moonrabbit_scale_codec::Input for ShardInput<'a, I> {
	fn remaining_len(&mut self) -> Result<Option<usize>, moonrabbit_scale_codec::Error> {
		Ok(Some(self.remaining_len))
	}

	fn read(&mut self, into: &mut [u8]) -> Result<(), moonrabbit_scale_codec::Error> {
...
mod tests {
	use super::*;
	use moonrabbit_primitives::v0::{AvailableData, BlockData, PoVBlock};
