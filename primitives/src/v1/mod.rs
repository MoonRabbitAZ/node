...
pub use moonrabbit_core_primitives::v1::{
...
// Export some moonrabbit-parachain primitives
pub use moonrabbit_parachain::primitives::{
...
#[cfg(feature = "std")]
use moonrabbit_util_mem::{MallocSizeOf, MallocSizeOfOps};
...
pub mod well_known_keys {
	...
	use sp_std::prelude::*;
	use moonrabbit_scale_codec::Encode as _;
	...
	///
	/// The storage entry stores a `Hash`. This is moonrabbit hash which is at the moment
	/// `blake2b-256`.
...
/// Custom validity errors used in moonrabbit while validating transactions.
#[repr(u8)]
...
/// Consensus engine id for moonrabbit v1 consensus engine.
pub const moonrabbit_ENGINE_ID: runtime_primitives::ConsensusEngineId = *b"POL1";

/// A consensus log item for moonrabbit validation. To be used with [`moonrabbit_ENGINE_ID`].
...
impl ConsensusLog {
	...
	pub fn from_digest_item<H>(digest_item: &runtime_primitives::DigestItem<H>)
		-> Result<Option<Self>, moonrabbit_scale_codec::Error>
	{
		match digest_item {
			runtime_primitives::DigestItem::Consensus(id, encoded) if id == &moonrabbit_ENGINE_ID =>
...
impl<H> From<ConsensusLog> for runtime_primitives::DigestItem<H> {
	fn from(c: ConsensusLog) -> runtime_primitives::DigestItem<H> {
		Self::Consensus(moonrabbit_ENGINE_ID, c.encode())
	}
}
