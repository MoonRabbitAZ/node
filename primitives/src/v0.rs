...
pub use moonrabbit_core_primitives::*;
pub use moonrabbit_scale_codec::Compact;
...
impl moonrabbit_scale_codec::Encode for CompactStatement {
	fn size_hint(&self) -> usize {
		// magic + discriminant + payload
		4 + 1 + 32
	}

	fn encode_to<T: moonrabbit_scale_codec::Output + ?Sized>(&self, dest: &mut T) {
		dest.write(&BACKING_STATEMENT_MAGIC);
		CompactStatementInner::from(self.clone()).encode_to(dest)
	}
}

impl moonrabbit_scale_codec::Decode for CompactStatement {
	fn decode<I: moonrabbit_scale_codec::Input>(input: &mut I) -> Result<Self, moonrabbit_scale_codec::Error> {
		let maybe_magic = <[u8; 4]>::decode(input)?;
		if maybe_magic != BACKING_STATEMENT_MAGIC {
			return Err(moonrabbit_scale_codec::Error::from("invalid magic string"));
		}
