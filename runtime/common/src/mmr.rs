	...
	T: pallet_beefy::Config,
{
	fn on_new_root(root: &<T as pallet_mmr::Config>::Hash) {
		let digest = sp_runtime::generic::DigestItem::Consensus(
			...
			moonrabbit_scale_codec::Encode::encode(
...
pub struct MmrLeaf<BlockNumber, Hash, MerkleRoot> {
	...
	/// A merkle root of all registered jurisdiction heads.
...
pub trait Config: pallet_mmr::Config + pallet_beefy::Config {
	...
	/// Retrieve a list of current jurisdiction heads.
	///
	/// The trait is implemented for `paras` module, but since not all chains might have jurisdictions,
...
/// A type that is able to return current list of jurisdiction heads that end up in the MMR leaf.
