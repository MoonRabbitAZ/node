...
use moonrabbit_node_primitives::approval::{
	self as approval_types, BlockApprovalMeta, RelayVRFStory,
};
use moonrabbit_node_jaeger as jaeger;
...
async fn imported_block_info(
	...
	let babe_epoch = {
		...
		//
		// It's worth nothing that moonrabbit session changes, at least for the purposes of parachains,
...
pub(crate) async fn handle_new_head(
	...
	for (block_hash, block_header, imported_block_info) in imported_blocks_and_info {
		...
		// cf. https://github.com/moonRabbitAZ/moonrabbit/issues/2411
...
mod tests {
	use super::*;
	use moonrabbit_node_subsystem_test_helpers::make_subsystem_context;
	use moonrabbit_node_primitives::approval::{VRFOutput, VRFProof};
	use moonrabbit_primitives::v1::ValidatorIndex;
	use moonrabbit_node_subsystem::messages::AllMessages;
	...
	impl AssignmentCriteria for MockAssignmentCriteria {
		fn compute_assignments(
			...
			_relay_vrf_story: moonrabbit_node_primitives::approval::RelayVRFStory,
			_config: &criteria::Config,
			_leaving_cores: Vec<(CandidateHash, moonrabbit_primitives::v1::CoreIndex, moonrabbit_primitives::v1::GroupIndex)>,
		) -> HashMap<moonrabbit_primitives::v1::CoreIndex, criteria::OurAssignment> {
			HashMap::new()
		}

		fn check_assignment_cert(
			&self,
			_claimed_core_index: moonrabbit_primitives::v1::CoreIndex,
			_validator_index: moonrabbit_primitives::v1::ValidatorIndex,
			_config: &criteria::Config,
			_relay_vrf_story: moonrabbit_node_primitives::approval::RelayVRFStory,
			_assignment: &moonrabbit_node_primitives::approval::AssignmentCert,
			_backing_group: moonrabbit_primitives::v1::GroupIndex,
		) -> Result<moonrabbit_node_primitives::approval::DelayTranche, criteria::InvalidAssignment> {
			Ok(0)
		}
	}
