// Copyright 2020-2021 moonrabbit Technologies (UK) Ltd.
...
use moonrabbit_primitives::v1::{Block, Hash};
use moonrabbit_subsystem::{SubsystemError, SubsystemResult};
...
pub trait Network: Clone + Send + 'static {
	...
	/// to the moonrabbit protocol - the user of this function should filter only for events related
...
impl Network for Arc<NetworkService<Block, Hash>> {
	fn event_stream(&mut self) -> BoxStream<'static, NetworkEvent> {
		NetworkService::event_stream(self, "moonrabbit-network-bridge").boxed()
	}
