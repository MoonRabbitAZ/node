...
//! Overview over request/responses as used in `moonrabbit`.
//!
...
use moonrabbit_primitives::v1::MAX_CODE_SIZE;
use strum::EnumIter;
...
impl Protocol {
	...
	pub const fn get_protocol_name_static(self) -> &'static str {
		match self {
			Protocol::ChunkFetching => "/moonrabbit/req_chunk/1",
			Protocol::CollationFetching => "/moonrabbit/req_collation/1",
			Protocol::PoVFetching => "/moonrabbit/req_pov/1",
			Protocol::AvailableDataFetching => "/moonrabbit/req_available_data/1",
			Protocol::StatementFetching => "/moonrabbit/req_statement/1",
		}
	}
}
