...
//! moonrabbit service. Specialized wrapper over substrate service.
...
use {
	std::time::Duration,
	tracing::info,
	moonrabbit_network_bridge::RequestMultiplexer,
	...
	moonrabbit_overseer::{AllSubsystems, BlockInfo, Overseer, OverseerHandler},
	moonrabbit_primitives::v1::ParachainHost,
...
pub use chain_spec::{moonrabbitChainSpec, MoonrabbitChainSpec};
pub use consensus_common::{Proposal, SelectChain, BlockImport, block_validation::Chain};
pub use moonrabbit_primitives::v1::{Block, BlockId, CollatorPair, Hash, Id as ParaId};
...
native_executor_instance!(
	pub moonrabbitExecutor,
	moonrabbit_runtime::api::dispatch,
	moonrabbit_runtime::native_version,
...
impl IdentifyVariant for Box<dyn ChainSpec> {
	fn is_moonrabbit(&self) -> bool { self.id().starts_with("moonrabbit") || self.id().starts_with("aaa") }
	fn is_dev(&self) -> bool {
		self.id().ends_with("dev")
	}
}

// If we're using prometheus, use a registry with a prefix of `moonrabbit`.
fn set_prometheus_registry(config: &mut Configuration) -> Result<(), Error> {
	if let Some(PrometheusConfig { registry, .. }) = config.prometheus_config.as_mut() {
		*registry = Registry::new_custom(Some("moonrabbit".into()), None)?;
	}
...
fn new_partial<RuntimeApi, Executor>(
	...
	service::PartialComponents<
		...
		(
			impl Fn(
				moonrabbit_rpc::DenyUnsafe,
				moonrabbit_rpc::SubscriptionTaskExecutor,
			) -> moonrabbit_rpc::RpcExtension,
			(
		...
		Executor: NativeExecutionDispatch + 'static,
{
	...
	let rpc_extensions_builder = {
		...
		move |deny_unsafe, subscription_executor: moonrabbit_rpc::SubscriptionTaskExecutor|
			-> moonrabbit_rpc::RpcExtension
		{
			...
			moonrabbit_rpc::create_full(deps)
		}
	};
...
{
	use moonrabbit_node_subsystem_util::metrics::Metrics;

	use moonrabbit_availability_distribution::AvailabilityDistributionSubsystem;
	use moonrabbit_node_core_av_store::AvailabilityStoreSubsystem;
	use moonrabbit_availability_bitfield_distribution::BitfieldDistribution as BitfieldDistributionSubsystem;
	use moonrabbit_node_core_bitfield_signing::BitfieldSigningSubsystem;
	use moonrabbit_node_core_backing::CandidateBackingSubsystem;
	use moonrabbit_node_core_candidate_selection::CandidateSelectionSubsystem;
	use moonrabbit_node_core_candidate_validation::CandidateValidationSubsystem;
	use moonrabbit_node_core_chain_api::ChainApiSubsystem;
	use moonrabbit_node_collation_generation::CollationGenerationSubsystem;
	use moonrabbit_collator_protocol::{CollatorProtocolSubsystem, ProtocolSide};
	use moonrabbit_network_bridge::NetworkBridge as NetworkBridgeSubsystem;
	use moonrabbit_node_core_provisioner::ProvisioningSubsystem as ProvisionerSubsystem;
	use moonrabbit_node_core_runtime_api::RuntimeApiSubsystem;
	use moonrabbit_statement_distribution::StatementDistribution as StatementDistributionSubsystem;
	use moonrabbit_availability_recovery::AvailabilityRecoverySubsystem;
	use moonrabbit_approval_distribution::ApprovalDistribution as ApprovalDistributionSubsystem;
	use moonrabbit_node_core_approval_voting::ApprovalVotingSubsystem;
	use moonrabbit_gossip_support::GossipSupport as GossipSupportSubsystem;
		...
		Executor: NativeExecutionDispatch + 'static,
{
	...
	let overseer_handler = if let Some((authority_discovery_service, keystore)) = maybe_params {
		...
		task_manager.spawn_essential_handle().spawn_blocking("overseer", Box::pin(async move {
			...
			let forward = moonrabbit_overseer::forward_events(overseer_client, overseer_handler_clone);
		...
		Dispatch: NativeExecutionDispatch + 'static,
{
	...
	let light_deps = moonrabbit_rpc::LightDeps {
	...
	let rpc_extensions = moonrabbit_rpc::create_light(light_deps);
...
>
{
	...
	if config.chain_spec.is_moonrabbit() {
		let service::PartialComponents { client, backend, import_queue, task_manager, .. }
			= new_partial::<moonrabbit_runtime::RuntimeApi, MoonrabbitExecutor>(config, jaeger_agent, None)?;
		Ok((Arc::new(Client::Moonrabbit(client)), backend, import_queue, task_manager))
	} else {
		let service::PartialComponents { client, backend, import_queue, task_manager, .. }
			= new_partial::<moonrabbit_runtime::RuntimeApi, moonrabbitExecutor>(config, jaeger_agent, None)?;
		Ok((Arc::new(Client::moonrabbit(client)), backend, import_queue, task_manager))
...
pub fn build_light(config: Configuration) -> Result<(
	...
	if config.chain_spec.is_moonrabbit() {
		new_light::<moonrabbit_runtime::RuntimeApi, MoonrabbitExecutor>(config)
	} else {
		new_light::<moonrabbit_runtime::RuntimeApi, moonrabbitExecutor>(config)
	}
}
...
pub fn build_full(
	...
	if config.chain_spec.is_moonrabbit() {
		new_full::<moonrabbit_runtime::RuntimeApi, MoonrabbitExecutor>(
			config,
			is_collator,
			grandpa_pause,
			disable_beefy,
