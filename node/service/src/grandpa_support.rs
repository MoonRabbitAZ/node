...
//! moonrabbit-specific GRANDPA integration utilities.
...
use {
	moonrabbit_primitives::v1::{Block as moonrabbitBlock, Header as moonrabbitHeader},
	moonrabbit_subsystem::messages::ApprovalVotingMessage,
	prometheus_endpoint::{self, Registry},
	moonrabbit_overseer::OverseerHandler,
...
#[cfg(feature = "full-node")]
impl<B> grandpa::VotingRule<moonrabbitBlock, B> for ApprovalCheckingVotingRule
	where B: sp_blockchain::HeaderBackend<moonrabbitBlock>
{
	fn restrict_vote(
		&self,
		_backend: Arc<B>,
		base: &moonrabbitHeader,
		best_target: &moonrabbitHeader,
		current_target: &moonrabbitHeader,
	) -> grandpa::VotingRuleResult<moonrabbitBlock> {
...
pub(crate) fn kusama_hard_forks() -> Vec<(
	...
	(Hash, moonrabbit_primitives::v1::BlockNumber),
...
mod tests {
	...
	use moonrabbit_test_client::{
		TestClientBuilder, TestClientBuilderExt, DefaultTestClientBuilderExt, InitmoonrabbitBlockBuilder,
	...
	fn grandpa_pause_voting_rule_works() {
		...
		let mut push_blocks = {
			...
			move |n| {
				for _ in 0..n {
					let block = client.init_moonrabbit_block_builder().build().unwrap().block;
