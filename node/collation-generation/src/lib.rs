...
//! The collation generation subsystem is the interface between moonrabbit and the collators.
...
use moonrabbit_node_primitives::{
...
use moonrabbit_node_subsystem_util::{
...
impl CollationGenerationSubsystem {
	...
	where
		Context: SubsystemContext<Message = CollationGenerationMessage>,
	{
		use moonrabbit_node_subsystem::ActiveLeavesUpdate;
		use moonrabbit_node_subsystem::FromOverseer::{Communication, Signal};
		use moonrabbit_node_subsystem::OverseerSignal::{ActiveLeaves, BlockFinalized, Conclude};
		...
		for (core_idx, core) in availability_cores.into_iter().enumerate() {
			...
			let (scheduled_core, assumption) = match core {
				...
				CoreState::Occupied(_occupied_core) => {
					// TODO: https://github.com/moonRabbitAZ/moonrabbit/issues/1573
					tracing::trace!(
						target: LOG_TARGET,
						core_idx = %core_idx,
			...
			ctx.spawn("collation generation collation builder", Box::pin(async move {
				...
				let pov = {
					let pov = moonrabbit_node_primitives::maybe_compress_pov(collation.proof_of_validity);
...
fn erasure_root(
	...
	let chunks = moonrabbit_erasure_coding::obtain_chunks_v1(n_validators, &available_data)?;
	Ok(moonrabbit_erasure_coding::branches(&chunks).root())
}
...
mod tests {
	mod handle_new_activations {
		...
		use moonrabbit_node_primitives::{Collation, CollationResult, BlockData, PoV, POV_BOMB_LIMIT};
		use moonrabbit_node_subsystem::messages::{
			AllMessages, RuntimeApiMessage, RuntimeApiRequest,
		};
		use moonrabbit_node_subsystem_test_helpers::{
