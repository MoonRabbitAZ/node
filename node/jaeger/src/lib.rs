...
//! moonrabbit Jaeger related primitives
//!
//! Provides primitives used by moonrabbit for interfacing with Jaeger.
//!
//! # Integration
//!
...
impl Jaeger {
	...
	pub fn launch<S: SpawnNamed>(self, spawner: S) -> result::Result<(), JaegerError> {
		...
		let (traces_in, mut traces_out) =
			mick_jaeger::init(mick_jaeger::Config { service_name: format!("moonrabbit-{}", cfg.node_name) });
