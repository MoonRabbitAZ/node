...
use moonrabbit_primitives::v1::{ValidatorId, AccountId, AssignmentId};
use moonrabbit_service::chain_spec::{get_account_id_from_seed, get_from_seed, Extensions};
use moonrabbit_test_runtime::{constants::currency::DOTS, BABE_GENESIS_EPOCH_CONFIG};
...
/// The `ChainSpec` parametrized for moonrabbit test runtime.
pub type moonrabbitChainSpec =
	service::GenericChainSpec<moonrabbit_test_runtime::GenesisConfig, Extensions>;

/// Local testnet config (multivalidator Alice + Bob)
pub fn moonrabbit_local_testnet_config() -> moonrabbitChainSpec {
	moonrabbitChainSpec::from_genesis(
		"Local Testnet",
		"local_testnet",
		ChainType::Local,
		|| moonrabbit_local_testnet_genesis(),
		vec![],
		None,
...
pub fn moonrabbit_local_testnet_genesis() -> moonrabbit_test_runtime::GenesisConfig {
...
fn moonrabbit_testnet_genesis(
...
) -> moonrabbit_test_runtime::GenesisConfig {
	use moonrabbit_test_runtime as runtime;
	...
	runtime::GenesisConfig {
		...
		parachains_configuration: runtime::ParachainsConfigurationConfig {
			config: moonrabbit_runtime_parachains::configuration::HostConfiguration {
