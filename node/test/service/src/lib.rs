...
use moonrabbit_overseer::OverseerHandler;
use moonrabbit_primitives::v1::{
	Id as ParaId, HeadData, ValidationCode, Balance, CollatorPair,
};
use moonrabbit_runtime_common::BlockHashCount;
use moonrabbit_service::{
	Error, NewFull, FullClient, ClientHandle, ExecuteWithClient, IsCollator,
};
use moonrabbit_node_subsystem::messages::{CollatorProtocolMessage, CollationGenerationMessage};
use moonrabbit_test_runtime::{
	Runtime, SignedExtra, SignedPayload, VERSION, ParasSudoWrapperCall, SudoCall, UncheckedExtrinsic,
};
use moonrabbit_node_primitives::{CollatorFn, CollationGenerationConfig};
use moonrabbit_runtime_parachains::paras::ParaGenesisArgs;
...
native_executor_instance!(
	pub moonrabbitTestExecutor,
	moonrabbit_test_runtime::api::dispatch,
	moonrabbit_test_runtime::native_version,
	frame_benchmarking::benchmarking::HostFunctions,
);

/// The client type being used by the test service.
pub type Client = FullClient<moonrabbit_test_runtime::RuntimeApi, moonrabbitTestExecutor>;

pub use moonrabbit_service::FullBackend;
...
pub fn new_full(
	...
	Error,
> {
	moonrabbit_service::new_full::<moonrabbit_test_runtime::RuntimeApi, moonrabbitTestExecutor>(
		config,
		is_collator,
		None,
		true,
...
impl ClientHandle for TestClient {
	fn execute_with<T: ExecuteWithClient>(&self, t: T) -> T::Output {
		T::execute_with_client::<_, _, moonrabbit_service::FullBackend>(t, self.0.clone())
	}
}

/// Create a moonrabbit `Configuration`.
///
...
pub fn node_config(
	...
	let mut spec = moonrabbit_local_testnet_config();
	...
	Configuration {
		impl_name: "moonrabbit-test-node".to_string(),
...
pub fn run_validator_node(
	...
	let NewFull { task_manager, client, network, rpc_handlers, overseer_handler, .. } =
		new_full(config, IsCollator::No, worker_program_path).expect("could not create moonrabbit test service");
...
/// [`moonrabbitTestNode::register_collator`].
pub fn run_collator_node(
	...
	let NewFull {
		...
		.expect("could not create moonrabbit test service");
...
/// A moonrabbit test node instance used for testing.
pub struct moonrabbitTestNode {
...
impl moonrabbitTestNode {
	/// Send an extrinsic to this node.
	pub async fn send_extrinsic(
		&self,
		function: impl Into<moonrabbit_test_runtime::Call>,
...
pub fn construct_extrinsic(
	...
	UncheckedExtrinsic::new_signed(
		function.clone(),
		moonrabbit_test_runtime::Address::Id(caller.public().into()),
		moonrabbit_primitives::v0::Signature::Sr25519(signature.clone()),
		extra.clone(),
	)
}
...
pub fn construct_transfer_extrinsic(
	...
	let function = moonrabbit_test_runtime::Call::Balances(
