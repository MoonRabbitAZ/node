// Copyright 2020 moonrabbit Technologies (UK) Ltd.
...
use alloc::vec::Vec;
use moonrabbit_scale_codec::{self, Encode, Decode};
...
pub enum BodyId {
	...
	Named(Vec<u8>),
	/// An indexed body.
	// TODO: moonrabbit-scale-codec#262: Change to be a tuple.
	Index { #[codec(compact)] id: u32 },
	/// The unambiguous executive body (for moonrabbit, this would be the moonrabbit council).
	Executive,
	/// The unambiguous technical body (for moonrabbit, this would be the Technical Committee).
	Technical,
	/// The unambiguous legislative body (for moonrabbit, this could be considered the opinion of a majority of
	/// lock-voters).
	Legislative,
	/// The unambiguous judicial body (this doesn't exist on moonrabbit, but if it were to get a "grand oracle", it
...
pub enum Junction {
	...
	///
	/// Generally used when the context is a moonrabbit Relay-chain.
