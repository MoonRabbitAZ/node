...
use moonrabbit_node_primitives::{Collation, CollatorFn, CollationResult, Statement, SignedFullStatement};
use moonrabbit_primitives::v1::{CollatorId, CollatorPair};
...
impl Collator {
	...
	pub fn create_collation_function(&self, spawner: impl SpawnNamed + Clone + 'static) -> CollatorFn {
		...
		Box::new(move |relay_parent, validation_data| {
			...
			let compressed_pov = moonrabbit_node_primitives::maybe_compress_pov(pov);
...
mod tests {
	...
	use moonrabbit_parachain::{primitives::{ValidationParams, ValidationResult}};
	use moonrabbit_primitives::v1::PersistedValidationData;
	...
	fn validate_collation(
		...
		use moonrabbit_node_core_pvf::testing::validate_candidate;
